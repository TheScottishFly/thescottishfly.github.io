<template>
  <div v-if="this.getIntro() === false" class="flex flex-col items-center justify-center w-full h-full pt-12">
    <div class="flex flex-col items-center justify-center p-4 md:mt-24 sm:mt-12 mt-6">
      <a href="mailto:contact@gilianrosnet.fr" class="flex flex-row items-center justify-center m-2">
        <img src="data:image/png;base64,">
        <p class="ml-2">contact@gilianrosnet.fr</p>
      </a>
      <a href="https://www.malt.fr/profile/gilianrosnetboutinet" target="_blank" class="flex flex-row items-center justify-center m-2">
        <img src="../assets/img/malt_picto_color_png.png" alt="Logo malt" class="w-12">
        <p class="ml-2">gilianrosnetboutinet</p>
      </a>
      <a href="https://www.linkedin.com/in/gilian-rosnet/" target="_blank" class="flex flex-row items-center justify-center m-2">
        <svg xmlns="http://www.w3.org/2000/svg" x="0px" y="0px"
             width="62" height="62"
             viewBox="0 0 48 48"
             style=" fill:#000000;"><g id="surface1"><path style=" fill:#0288D1;" d="M 42 37 C 42 39.761719 39.761719 42 37 42 L 11 42 C 8.238281 42 6 39.761719 6 37 L 6 11 C 6 8.238281 8.238281 6 11 6 L 37 6 C 39.761719 6 42 8.238281 42 11 Z "></path><path style=" fill:#FFFFFF;" d="M 12 19 L 17 19 L 17 36 L 12 36 Z "></path><path style=" fill:#FFFFFF;" d="M 14.484375 17 L 14.457031 17 C 12.964844 17 12 15.886719 12 14.5 C 12 13.078125 12.996094 12 14.515625 12 C 16.035156 12 16.972656 13.078125 17 14.5 C 17 15.886719 16.035156 17 14.484375 17 Z "></path><path style=" fill:#FFFFFF;" d="M 36 36 L 31 36 L 31 26.902344 C 31 24.703125 29.773438 23.203125 27.808594 23.203125 C 26.308594 23.203125 25.496094 24.214844 25.101563 25.191406 C 24.957031 25.542969 25 26.511719 25 27 L 25 36 L 20 36 L 20 19 L 25 19 L 25 21.617188 C 25.722656 20.5 26.851563 19 29.738281 19 C 33.316406 19 36 21.25 36 26.273438 Z "></path></g></svg>
        <p class="ml-2">gilian-rosnet</p>
      </a>
      <a href="https://github.com/TheScottishFly" target="_blank" class="flex flex-row items-center justify-center m-2">
        <i class="devicon-github-plain colored text-5xl"></i>
        <p class="ml-2">TheScottishFly</p>
      </a>
    </div>
  </div>
</template>

<script>
  import { mapGetters, mapMutations } from 'vuex'

  export default {
    name: 'Contact',
    methods: {
      ...mapGetters([
        'getIntro',
        'getBgApp',
        'getTyped'
      ]),
      ...mapMutations([
        'finishIntro',
        'changeTyped'
      ]),

    },
  };
</script>

<style scoped>
</style>
